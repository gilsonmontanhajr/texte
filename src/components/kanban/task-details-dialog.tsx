...
import { X, Plus, Send, User as UserIcon, Check, ChevronsUpDown } from "lucide-react";
...
export function TaskDetailsDialog({ task, open, onClose, onUpdate, currentUser, collaborators, readOnly }: TaskDetailsDialogProps) {
    ...
    useEffect(() => {
        if (!task || readOnly) return; // Disable auto-save in read-only
    ...
    const handleUpdateAssignee = () => {
        // ... (this function wasn't used in the original either directly, logic was inline in CommandItem)
    };
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className="max-w-4xl h-[80vh] flex flex-col p-0 gap-0 z-[9999]">
